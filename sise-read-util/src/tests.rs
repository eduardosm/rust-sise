...
fn test_node_as_atom() {
    ...
    let read_util = crate::NodeReadUtil::new(&node, None);
...
fn test_node_as_list() {
    let node = sise_expr!([]);
    let read_util = crate::NodeReadUtil::new(&node, None);
    assert_eq!(read_util.as_list().unwrap().list(), [] as [sise::Node; 0]);
...
fn test_atom_decode() {
    ...
    let node_read_util = crate::NodeReadUtil::new(&node, None);
...
fn test_list_expect_end() {
    ...
    let node_read_util = crate::NodeReadUtil::new(&node, None);
    let list_read_util = node_read_util.as_list().unwrap();
    let expected_err = crate::ReadError::ExpectedListEnd {
        node_pos: None,
    };
...
fn test_list_next_item() {
    ...
    let expected_err = crate::ReadError::ExpectedNodeInList {
        list_pos: None,
    };
